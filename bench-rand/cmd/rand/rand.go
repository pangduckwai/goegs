...
import (
	...
	"sea9.org/go/egs/randBench/pkg/rand"
)
...
func main() {
	...
	case 1:
	default:
		log.Println("Usage: cmd/rand [range] [num-of-runs]")
	}

	lps, cnt, nmz := rand.Sim(1, rng, run)
	...
	prt.Printf("[RAND] %v simulations with [0,%v) range, elapsed time: %12v (%v per op)\n%v", run, rng, lps, lps/time.Duration(run), buf.String())
